export default {
  async fetch(request, env, ctx) {
    ...
    const service_account_email = env.GCS_SERVICE_ACCOUNT_EMAIL;
    ...
    const oauth_response = await fetch(
    ...
    try {
      ...
      const queryCacheKey =
        `${newUrl.hostname}${newUrl.pathname}${newUrl.search}`;
      ...
      const { ...cache } = cacheAssets.find(({ regex }) =>
        newUrl.pathname.match(regex)
      ) ?? {};
